package ro.pub.cs.systems.eim.lab08.chatserviceandroidnsd.networkserviceoperations;
...
public class NetworkServiceDiscoveryOperations {
	...
	public NetworkServiceDiscoveryOperations(final Context context) {
		...
		resolveListener = new ResolveListener() {
			...
			public void onServiceResolved(NsdServiceInfo nsdServiceInfo) {
				...
				NetworkService networkService = new NetworkService(nsdServiceInfo.getServiceName(), host, port,
		...
		discoveryListener = new NsdManager.DiscoveryListener() {
			...
			public void onServiceLost(final NsdServiceInfo nsdServiceInfo) {
				...
				handler.post(new Runnable() {
					...
					public void run() {
						ArrayList<NetworkService> discoveredServices = chatActivity.getDiscoveredServices();
						NetworkService networkService = new NetworkService(nsdServiceInfo.getServiceName(),
								(nsdServiceInfo.getHost() != null) ? nsdServiceInfo.getHost().toString() : null,
								nsdServiceInfo.getPort(), -1);
						if (discoveredServices.contains(networkService)) {
						...
						Log.d(Constants.TAG, "serviceName = " + serviceName + "nsdServiceInfo.getServiceName() = "
	...
	public void registerNetworkService(int port) throws Exception {
		...
		nsdServiceInfo
				.setServiceName(Constants.SERVICE_NAME + Utilities.generateIdentifier(Constants.IDENTIFIER_LENGTH));
	...
	public void setCommunicationFromClients(List<ChatClient> communicationFromClients) {
		...
		for (ChatClient communicationFromClient : communicationFromClients) {
			NetworkService conversation = new NetworkService(null,
					communicationFromClient.getSocket().getInetAddress().toString(),
					communicationFromClient.getSocket().getLocalPort(), Constants.CONVERSATION_FROM_CLIENT);
