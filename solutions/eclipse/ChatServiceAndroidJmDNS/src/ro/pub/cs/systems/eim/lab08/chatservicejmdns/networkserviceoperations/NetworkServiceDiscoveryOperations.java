...
public class NetworkServiceDiscoveryOperations {
	...
	public NetworkServiceDiscoveryOperations(final Context context) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					InetAddress address = InetAddress.getByAddress(ByteBuffer.allocate(4)
							.putInt(Integer.reverseBytes(wifiManager.getConnectionInfo().getIpAddress())).array());
		...
		serviceListener = new ServiceListener() {
			...
			public void serviceResolved(ServiceEvent serviceEvent) {
				...
				NetworkService networkService = new NetworkService(serviceEvent.getName(), host, port,
			...
			public void serviceRemoved(final ServiceEvent serviceEvent) {
				...
				handler.post(new Runnable() {
					...
					public void run() {
						ArrayList<NetworkService> discoveredServices = chatActivity.getDiscoveredServices();
						NetworkService networkService = new NetworkService(serviceEvent.getName(), finalizedHost,
								finalizedPort, -1);
						if (discoveredServices.contains(networkService)) {
	...
	public void registerNetworkService(int port) throws Exception {
		...
		ServiceInfo serviceInfo = ServiceInfo.create(Constants.SERVICE_TYPE,
				Constants.SERVICE_NAME + Utilities.generateIdentifier(Constants.IDENTIFIER_LENGTH), port,
